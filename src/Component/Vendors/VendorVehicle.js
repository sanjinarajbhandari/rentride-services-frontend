...
const VendorVehicle = () => {
  ...
  // Filter vehicles to only show the current user's vehicles
  const userVehicles = vehicle.filter(x => x.userId === user?.rest?._id);
  
  // Get current vehicles for the current page
  const indexOfLastVehicle = currentPage * vehiclesPerPage;
  const indexOfFirstVehicle = indexOfLastVehicle - vehiclesPerPage;
  const currentVehicles = userVehicles.slice(
    indexOfFirstVehicle,
    indexOfLastVehicle
  );
    ...
    <div className="min-h-[calc(100vh-320px)]">
      ...
      {userVehicles.length > 0 && (
        <div className="pagination flex justify-center items-center gap-2 my-5">
          ...
          {Array.from(
            { length: Math.ceil(userVehicles.length / vehiclesPerPage) },
            (_, i) => (
              <button
                key={i}
                onClick={() => paginate(i + 1)}
          ...
          <button
            onClick={() =>
              currentPage < Math.ceil(userVehicles.length / vehiclesPerPage) &&
              paginate(currentPage + 1)
            }
            disabled={currentPage === Math.ceil(userVehicles.length / vehiclesPerPage)}
            className={`w-10 h-10 flex items-center justify-center rounded-full border transition ${
              currentPage === Math.ceil(userVehicles.length / vehiclesPerPage)
                ? "bg-gray-300 text-gray-500 cursor-not-allowed"
                : "bg-indigo-500 text-white hover:bg-indigo-600"
